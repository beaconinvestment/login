...
class FrontEndController extends JoshController
{
    ...
    public function postLogin(Request $request)
    {
        ...
        try {
            ...
            //if (($user = Sentinel::authenticate($request->only(['email', 'password']), $request->get('remember-me', 0))) || ($user = Sentinel::authenticate($request->only(['cell_no', 'password']), $request->get('remember-me', 0)))) {
            if ($user = Sentinel::authenticate($request->only(['cell_no', 'password']), $request->get('remember-me', 0))){
...
//            $b_date = $book->booking_date;
//            $i_amount = $book->total_price;
//            $confirm_after = $book->confirmation_after;
...
//                print_r($payments);
//                $p_date[] = $pay->created_at->format('d-M-Y');
//            }
//        }
//        $pays = Booking::find($b_id);
//        foreach ($pays->payments as $pay) {
...
//            $p_date[] = $pay->created_at->format('d-M-Y');
//        }
//        $bookme = DB::table('booking')->first();
//        $pay = DB::table('payments')->first();
//        $t_rows = $bookme->total_installments;
//        $b_date = Carbon::parse($bookme->booking_date)->format('d/m/Y');
//        $b_date = Carbon::parse($bookme->booking_date)->format('Y/m/d');
//        $i_amount = $bookme->total_price;
//        $confirm_after = $bookme->confirmation_after;
//        $payment = $pay->payment;
...
//            't_rows','b_date','i_amount','payments','p_date','confirm_after','userid','pays','b_id'));
        return view('user_account', compact('user','books' ,'countries','recents','blogs','populars','counts'));
    }
    ...
    public function get_Ajax(Request $request)
    {
        $b_id = $request->input('b_id');
            $bookAll = DB::table('bookings')->where('id','=', $b_id)->first();
                $bookDate = $bookAll->booking_date;
                $confirm_after = $bookAll->confirmation_after;
                $i_amount = $bookAll->total_price;
                $t_row = $bookAll->total_installments;
                $ins_amount = $bookAll->installment_amount;
        ...
        return response()->json([$paymentss, $p_dates, $bookDate, $confirm_after, $i_amount,$t_row,$ins_amount]);
    }
...
//            $user->password = Hash::make($password);
    ...
    public function postRegister(UserRequest $request)
    {
        ...
        try {
            // Register the user
            $user = Sentinel::register($request->only(['first_name', 'last_name','cell_no','cnic','ref_by', 'email', 'password', 'gender']), $activate);
