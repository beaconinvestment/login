...
//            if ($user = Sentinel::authenticate($request->only(['email', 'password']), $request->get('remember-me', false))) {
            if (($user = Sentinel::authenticate($request->only(['email', 'password']), $request->get('remember-me', 0)))
                || ($user = Sentinel::authenticate($request->only(['cell_no', 'password']), $request->get('remember-me', 0))))
            {
//            if ($user = Sentinel::authenticate($request->only(['cell_no', 'password']), $request->get('remember-me', 0))) {
            ...
            $this->messageBag->add('cell_no' || 'email', trans('auth/message.account_not_found'));

        } catch (NotActivatedException $e) {
            $this->messageBag->add('cell_no' || 'email', trans('auth/message.account_not_activated'));
        } catch (ThrottlingException $e) {
            $delay = $e->getDelay();
            $this->messageBag->add('cell_no' || 'email', trans('auth/message.account_suspended', compact('delay')));
        }
